...
test('Stellar core config modal should open', async ({ page }) => {
  ...
  // This is a direct child link, not nested inside dropdowns
  const stellarCoreLink = page.locator('.sb-nav-item').filter({ hasText: 'Stellar core config' }).locator('a, div[role="button"]').first();
  ...
  await page.screenshot({ path: 'test-results/after-modal-click.png', fullPage: true });
