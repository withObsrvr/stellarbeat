...
import { QuorumSetService } from '../../../src/federated-voting/QuorumSetService';
...
describe('PhaseTransitioner', () => {
	...
	const setupState = (publicKey: string) => {
		return new FederatedVotingState(publicKey, new QuorumSet(1, ['B'], []));
	};
	...
	describe('tryAccept', () => {
		...
		it('should fail if no v-blocking set of nodes has accepted the statement and no quorum has ratified it', () => {
			...
			jest.spyOn(QuorumService, 'isQuorum').mockImplementation(() => null);
		...
		it('should accept if no v-blocking set of nodes has accepted the statement but a quorum has voted for it', () => {
			...
			const containsQuorumForVSpy = jest.spyOn(QuorumService, 'isQuorum');
	...
	describe('tryToMoveToConfirmPhase', () => {
		...
		it('should fail if agreement attempt is already in confirmed phase', () => {
			...
			federatedVotingState.phase = FederatedVotingPhase.confirmed;
			const isQuorumSpy = jest.spyOn(QuorumService, 'isQuorum');
		...
		it('should fail if vote(accept) is not ratified', () => {
			...
			expect(calls[0][2]).toEqual(new Map([['C', quorumSetOfC]]));
